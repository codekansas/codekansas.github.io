...
The forward-backward algorithm is used to find the marginal probability $p(h_i \| X)$. Assuming we know the emission probabilities $p(x_i \| h_i)$ and the transition probabilities $p(h_i \| h_{i - 1})$, the forward backward algorithm is given by the equations below.
...
$$\beta_{i - 1}^T = M \beta_i \qquad \beta_{N + 1} = \begin{cases}
    1 & \text{if } y = \text{stop} \\
    0 & \text{otherwise}
\end{cases}
$$
